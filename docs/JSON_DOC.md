...
`seed`<br/> ![Generic badge](https://img.shields.io/badge/Added_1.5.4-green.svg)  | Int&nbsp;*(can&nbsp;be&nbsp;`null`)* | Random seed used to derive the per-layer random seeds of all the Auto-layers of this level (`LayerInstance.seed`).<br/>		Changing it in the editor regenerates every per-layer seed. This is `null` in levels created before LDtk 1.5.4: in that case, the per-layer seeds are independent.
...
`identifier` | String | User defined unique identifier
`intGridSourceLayerDefUid`<br/><sup class="only">Only *IntGrid layers*</sup><br/><sup class="uidRef">UID ref to [Layer definition](#ldtk-LayerDefJson)</sup><br/> ![Generic badge](https://img.shields.io/badge/Added_1.5.4-green.svg)  | Int&nbsp;*(can&nbsp;be&nbsp;`null`)* | UID of the IntGrid layer used as source for automatic value subdivision
...
`tileRandomFlipX`<br/><sup class="internal">*Only used by editor*</sup><br/> ![Generic badge](https://img.shields.io/badge/Added_1.5.4-green.svg)  | Bool | If TRUE, the tile(s) rendered by this rule can be randomly flipped horizontally (this doesn't affect the pattern matching)
`tileRandomFlipY`<br/><sup class="internal">*Only used by editor*</sup><br/> ![Generic badge](https://img.shields.io/badge/Added_1.5.4-green.svg)  | Bool | If TRUE, the tile(s) rendered by this rule can be randomly flipped vertically (this doesn't affect the pattern matching)
